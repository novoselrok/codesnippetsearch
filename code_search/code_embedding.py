...
from code_search.serialize import add_extension
...
def build_annoy_indices(
        data_manager: DataManager, languages: List[str], n_trees=200, build_on_disk=False, verbose=True):
    for language in languages:
        if verbose:
        ...
        if build_on_disk:
            annoy_index.on_disk_build(add_extension('annoy', data_manager.get_language_annoy_index_path(language)))
...
def main():
    ...
    if args['code-embeddings']:
        build_code_embeddings(model, data_manager, shared.LANGUAGES, device)

    if args['annoy-index']:
        build_annoy_indices(data_manager, shared.LANGUAGES)
