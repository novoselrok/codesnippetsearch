...
def preprocess_code_tokens(tokens: Iterable[str]) -> shared.TokensGenerator:
    for token in tokens:
        ...
        if token_lower in CODE_PUNCTUATION or len(token_lower) == 0:
            continue
...
def preprocess_query_tokens(tokens: Iterable[str]) -> shared.TokensGenerator:
    for token in tokens:
        token_lemma = textblob.Word(token).lemmatize()
        token_lower = token_lemma.lower().strip()

        if token_lower in STOP_WORDS or token_lower in PUNCTUATION_TOKENS or len(token_lower) == 0:
            continue

        yield [token_lower]


def extract_doc(raw_doc, language, use_func_name_as_query=False):
    ...
    return {
        ...
        'code_tokens': remove_inline_comments(language, code_tokens),
        'url': raw_doc['url'],
    }
...
def prepare_set_docs(args):
    ...
    docs = (extract_doc(raw_doc, language) for raw_doc in utils.get_raw_docs(language, set_))
...
def prepare_language_vocabulary(args):
    ...
    preprocess_tokens_fn = preprocess_query_tokens if type_ == 'query' else preprocess_code_tokens
    docs = utils.load_cached_docs(language, 'train')
    tokens = utils.flatten(preprocess_tokens_fn(utils.flatten(doc[tokens_key] for doc in docs)))
...
def pad_encode_seqs(
        ...
        seqs: Callable[[], shared.TokensGenerator],
        max_length: int,
        language: str,
...
def pad_encode_query(query: str, language: str) -> np.ndarray:
    return pad_encode_seqs(
        ...
        lambda: (_ for _ in [query.split(' ')]), shared.QUERY_MAX_SEQ_LENGTH, language, 'query')
...
def prepare_set_seqs(args):
    ...
    # Prepare code seqs
    code_seqs = lambda: (doc['code_tokens'] for doc in utils.load_cached_docs(language, set_))
    padded_encoded_code_seqs = pad_encode_seqs(
        preprocess_code_tokens, code_seqs, shared.CODE_MAX_SEQ_LENGTH, language, 'code')

    # Prepare query seqs
    query_seqs = lambda: (doc['query_tokens'] for doc in utils.load_cached_docs(language, set_))
...
def prepare_evaluation_seqs(language):
    ...
    evaluation_code_seqs = lambda: (remove_inline_comments(language, doc['function_tokens'])
